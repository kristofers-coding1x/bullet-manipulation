...
local function createTab(name, icon)
    ...
    elseif #tabs == 3 then
        ...
        button.Position = UDim2.new(0, 0, 0, 75)
    end
end
...
aimbotContainer.Size = UDim2.new(0, 220, 0, 400)
aimbotContainer.Position = UDim2.new(0.5, -110, 0, 10)
...
local function createToggle(optionName, optionId, yPos, parent)
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        elseif optionId == "autoLock" then
            AimbotSettings.AutoLock = aimbotSettings[optionId]
        end
    end)
...
for i, option in ipairs(aimbotOptions) do
    createToggle(option.name, option.id, (i-1) * 25, aimbotContainer)
end
...
-- Create Sliders
createSlider("FOV", 50, 700, 350, 140, aimbotContainer, function(value)
    AimbotSettings.Fov = value
end)

createSlider("SMOOTHING", 0.1, 3, 1, 180, aimbotContainer, function(value)
    AimbotSettings.Smoothing = value
end)

createSlider("LOCK DISTANCE", 100, 2000, 1000, 220, aimbotContainer, function(value)
...
silentAimContainer.Size = UDim2.new(0, 220, 0, 500)
silentAimContainer.Position = UDim2.new(0.5, -110, 0, 10)
...
silentAimContainerCorner.CornerRadius = UDim.new(0, 8)
...
local silentAimOptions = {
    ...
    {name = "SHOW FOV", id = "showFov"},
    {name = "SNAP LINE", id = "snapLine"}
}

local silentAimToggles = {}

-- Create Silent Aim toggles
for i, option in ipairs(silentAimOptions) do
    local toggleFrame = Instance.new("Frame")
    toggleFrame.Size = UDim2.new(1, -10, 0, 22)
    toggleFrame.Position = UDim2.new(0, 5, 0, (i-1) * 25)
    toggleFrame.BackgroundColor3 = Color3.fromRGB(30, 0, 50)
    toggleFrame.BorderSizePixel = 0
    toggleFrame.ZIndex = 5
    ...
    silentAimToggles[option.id] = false

    toggleButton.MouseButton1Click:Connect(function()
        silentAimToggles[option.id] = not silentAimToggles[option.id]

        if silentAimToggles[option.id] then
            toggleButton.BackgroundColor3 = Color3.fromRGB(128, 0, 128)
            toggleIndicator.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            toggleIndicator.Position = UDim2.new(0, 11, 0, 1)
        else
        ...
        if option.id == "enableSilentAim" then
            silentAimSettings.Enabled = silentAimToggles[option.id]
        elseif option.id == "wallCheck" then
            silentAimSettings.WallCheck = silentAimToggles[option.id]
        elseif option.id == "prediction" then
            silentAimSettings.Prediction = silentAimToggles[option.id]
        elseif option.id == "showFov" then
            silentAimSettings.Fov.Visible = silentAimToggles[option.id]
        elseif option.id == "snapLine" then
            silentAimSettings.SnapLine.Visible = silentAimToggles[option.id]
        end
    end)
end

-- Silent Aim Sliders
createSlider("FOV RADIUS", 50, 300, 150, 140, silentAimContainer, function(value)
    silentAimSettings.Fov.Radius = value
end)

-- Hit Part Selection
local silentAimPartLabel = Instance.new("TextLabel")
silentAimPartLabel.Size = UDim2.new(1, -10, 0, 14)
silentAimPartLabel.Position = UDim2.new(0, 5, 0, 180)
silentAimPartLabel.BackgroundTransparency = 1
silentAimPartLabel.Text = "HIT PART"
silentAimPartLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
silentAimPartLabel.Font = Enum.Font.GothamBold
silentAimPartLabel.TextSize = 8
silentAimPartLabel.TextScaled = true
silentAimPartLabel.ZIndex = 5
silentAimPartLabel.Parent = silentAimContainer

local silentAimPartContainer = Instance.new("Frame")
silentAimPartContainer.Size = UDim2.new(1, -10, 0, 22)
silentAimPartContainer.Position = UDim2.new(0, 5, 0, 198)
silentAimPartContainer.BackgroundColor3 = Color3.fromRGB(30, 0, 50)
silentAimPartContainer.BorderSizePixel = 0
silentAimPartContainer.ZIndex = 5
silentAimPartContainer.Parent = silentAimContainer

local silentAimContainerCorner = Instance.new("UICorner")
silentAimContainerCorner.CornerRadius = UDim.new(0, 6)
silentAimContainerCorner.Parent = silentAimPartContainer

local silentAimParts = {"HEAD", "TORSO", "ROOT"}
local silentAimPartButtons = {}

for i, partName in ipairs(silentAimParts) do
    local button = Instance.new("TextButton")
    button.Size = UDim2.new(0, 65, 0, 18)
    button.Position = UDim2.new(0, 5 + (i-1) * 72, 0, 2)
    button.BackgroundColor3 = Color3.fromRGB(50, 0, 80)
    ...
    button.TextScaled = true
    button.ZIndex = 6
    button.Parent = silentAimPartContainer
    ...
    button.MouseButton1Click:Connect(function()
        for _, btn in ipairs(silentAimPartButtons) do
            btn.BackgroundColor3 = Color3.fromRGB(50, 0, 80)
            btn.TextColor3 = Color3.fromRGB(255, 0, 255)
        end
    ...
    table.insert(silentAimPartButtons, button)
end

-- Silent Aim Status
local silentAimStatus = Instance.new("TextLabel")
silentAimStatus.Size = UDim2.new(1, -10, 0, 30)
silentAimStatus.Position = UDim2.new(0, 5, 0, 230)
silentAimStatus.BackgroundTransparency = 1
silentAimStatus.Text = "STATUS: INACTIVE"
silentAimStatus.TextColor3 = Color3.fromRGB(255, 0, 0)
silentAimStatus.Font = Enum.Font.GothamBold
silentAimStatus.TextSize = 8
silentAimStatus.TextScaled = true
silentAimStatus.TextXAlignment = Enum.TextXAlignment.Center
silentAimStatus.TextYAlignment = Enum.TextYAlignment.Center
silentAimStatus.ZIndex = 5
silentAimStatus.Parent = silentAimContainer
...
local hookFunctionExists = pcall(function() return hookfunction end)
local drawingExists = pcall(function() return Drawing end)

pcall(function()
    bulletModule = require(replicatedStorage:WaitForChild("Modules"):WaitForChild("FPS"):WaitForChild("Bullet"))
end)

-- Silent Aim Drawing Objects
local silentAimDrawings = {}

-- Create Drawing Function for Silent Aim
local function silentAimDraw(drawingType, properties)
    ...
    silentAimDrawings[#silentAimDrawings + 1] = drawing
...
local silentAimFovCircle = silentAimDraw("Circle", {
    Visible = false,
    Filled = false,
    NumSides = 100,
    Color = Color3.fromRGB(255, 0, 0),
    Thickness = 2,
    Radius = silentAimSettings.Fov.Radius,
    Position = camera.ViewportSize / 2
})

-- Snap Line for Silent Aim
local silentAimSnapLine = silentAimDraw("Line", {
    Visible = false,
    Color = silentAimSettings.SnapLine.Color,
    Thickness = silentAimSettings.SnapLine.Thickness,
    Transparency = silentAimSettings.SnapLine.Transparency,
    From = Vector2.new(0, 0),
    To = Vector2.new(0, 0)
})

-- Silent Aim Utility Functions
...
local function getClosestPlayerSilentAim()
    ...
    for _, player in pairs(players:GetPlayers()) do
        ...
        if distance < closestDistance and distance <= silentAimSettings.Fov.Radius then
            closestDistance = distance
            closestPlayer = hitPart
        end
    end
...
-- Function to get closest player for snap line (no FOV restriction)
local function getClosestPlayerForSnapLineSilentAim()
    local closestPlayer, closestDistance = nil, math.huge
    local centerPos = camera.ViewportSize / 2
    ...
    for _, player in pairs(players:GetPlayers()) do
        ...
        local screenPosition, onScreen = camera:WorldToViewportPoint(hitPart.Position)
        -- We don't require onScreen for snap line
...
local oldBullet
if bulletModule and hookFunctionExists then
    oldBullet = hookfunction(bulletModule.CreateBullet, function(idk, model, model2, model3, aimPart, idk2, ammoType, tick, recoilPattern)
        if silentAimSettings.Enabled then
            local target = getClosestPlayerSilentAim()
            if target then
...
-- Silent Aim Update Functions
local function updateSilentAimStatus()
    if silentAimSettings.Enabled then
        silentAimStatus.Text = "STATUS: ACTIVE"
        silentAimStatus.TextColor3 = Color3.fromRGB(0, 255, 0)
    else
        silentAimStatus.Text = "STATUS: INACTIVE"
        silentAimStatus.TextColor3 = Color3.fromRGB(255, 0, 0)
    end
...
local function updateSilentAimFovVisibility()
    if silentAimFovCircle then
        silentAimFovCircle.Visible = silentAimSettings.Enabled and silentAimSettings.Fov.Visible
    end
end

local function updateSilentAimSnapLineVisibility()
    if silentAimSnapLine then
        silentAimSnapLine.Visible = silentAimSettings.Enabled and silentAimSettings.SnapLine.Visible
    end
end

-- Silent Aim Heartbeat Update
runService.Heartbeat:Connect(function()
    -- Update FOV circle
    if silentAimFovCircle and silentAimFovCircle.Visible then
        silentAimFovCircle.Position = camera.ViewportSize / 2
        silentAimFovCircle.Radius = silentAimSettings.Fov.Radius
    end
    
    -- Update snap line
    if silentAimSnapLine and silentAimSnapLine.Visible then
        local target = getClosestPlayerForSnapLineSilentAim()
        if target then
            local center = camera.ViewportSize / 2
            local screenPosition, onScreen = camera:WorldToViewportPoint(target.Position)
            
            -- Always show the snap line, even when target is off-screen
            silentAimSnapLine.From = center
            
            if onScreen then
                -- Target is on screen, draw directly to it
                silentAimSnapLine.To = Vector2.new(screenPosition.X, screenPosition.Y)
            else
                -- Target is off screen, calculate direction to screen edge
                ...
                if direction.Magnitude > 0 then
                    ...
                    if direction.X ~= 0 then
                        if direction.X > 0 then
                            tValues[1] = (viewportSize.X - center.X) / direction.X
                        else
                            tValues[1] = -center.X / direction.X
                        end
                    end
                    
                    if direction.Y ~= 0 then
                        if direction.Y > 0 then
                            tValues[2] = (viewportSize.Y - center.Y) / direction.Y
                        else
                            tValues[2] = -center.Y / direction.Y
                        end
                    end
                    
                    -- Find the smallest positive t value
                    local t = math.huge
                    for _, value in pairs(tValues) do
                        if value > 0 and value < t then
                            t = value
                        end
                    end
                    
                    -- If no valid t found, use a large value
                    if t == math.huge then
                        t = 1000
                    end
                    
                    -- Calculate edge point
                    local edgePoint = center + direction * t
                    silentAimSnapLine.To = edgePoint
                else
                    -- Fallback if direction is zero
                    silentAimSnapLine.To = center + Vector2.new(100, 0)
                end
            end
            
            silentAimSnapLine.Visible = true
        else
            silentAimSnapLine.Visible = false
        end
    end
end)

-- Initialize Silent Aim
updateSilentAimStatus()
updateSilentAimFovVisibility()
updateSilentAimSnapLineVisibility()
