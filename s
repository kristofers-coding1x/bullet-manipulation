...
local function createSlider(labelText, minVal, maxVal, defaultVal, yPos, parent, callback)
    ...
    sliderKnob.TouchTap:Connect(function(touch)
        dragging = true
        currentTouch = touch
        updateSlider(touch.Position)
    end)

    sliderKnob.TouchPan:Connect(function(touch)
        if dragging and touch == currentTouch then
            updateSlider(touch.Position)
        end
    end)
...
local function createVisualsToggle(optionName, optionId, yPos)
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        elseif optionId == "espBox" then
            espBoxSettings.Enabled = visualsSettings[optionId]
        end
    end)
...
-- ESP Box Color Button
local espBoxColorLabel = Instance.new("TextLabel")
espBoxColorLabel.Size = UDim2.new(1, -10, 0, 14)
espBoxColorLabel.Position = UDim2.new(0, 5, 0, 245)
espBoxColorLabel.BackgroundTransparency = 1
espBoxColorLabel.Text = "ESP BOX COLOR"
espBoxColorLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
espBoxColorLabel.Font = Enum.Font.GothamBold
espBoxColorLabel.TextSize = 8
espBoxColorLabel.TextScaled = true
espBoxColorLabel.ZIndex = 5
espBoxColorLabel.Parent = visualsContainer

local espBoxColorButton = Instance.new("TextButton")
espBoxColorButton.Size = UDim2.new(0, 200, 0, 22)
espBoxColorButton.Position = UDim2.new(0.5, -100, 0, 263)
espBoxColorButton.BackgroundColor3 = espBoxSettings.Color
espBoxColorButton.Text = ""
espBoxColorButton.ZIndex = 5
espBoxColorButton.Parent = visualsContainer

local espBoxColorCorner = Instance.new("UICorner")
espBoxColorCorner.CornerRadius = UDim.new(0, 6)
espBoxColorCorner.Parent = espBoxColorButton

espBoxColorButton.MouseButton1Click:Connect(function()
    ...
    local currentColor = espBoxSettings.Color
    ...
    espBoxSettings.Color = colors[nextColorIndex]
    espBoxColorButton.BackgroundColor3 = espBoxSettings.Color
end)
...
local function createESPBox(player)
    ...
    for _, line in pairs(espBoxes[player]) do
        line.Color = espBoxSettings.Color
        line.Thickness = espBoxSettings.Thickness
        line.Transparency = espBoxSettings.Transparency
        line.Visible = false
    end
end
...
local function updateESPBox(player)
    ...
    local corners = {
        topLeft = humanoidRootPart.CFrame * CFrame.new(-size.X/2, size.Y, -size.Z/2),
        topRight = humanoidRootPart.CFrame * CFrame.new(size.X/2, size.Y, -size.Z/2),
        bottomLeft = humanoidRootPart.CFrame * CFrame.new(-size.X/2, 0, -size.Z/2),
        bottomRight = humanoidRootPart.CFrame * CFrame.new(size.X/2, 0, -size.Z/2),
        topLeftBack = humanoidRootPart.CFrame * CFrame.new(-size.X/2, size.Y, size.Z/2),
        topRightBack = humanoidRootPart.CFrame * CFrame.new(size.X/2, size.Y, size.Z/2),
        bottomLeftBack = humanoidRootPart.CFrame * CFrame.new(-size.X/2, 0, size.Z/2),
        bottomRightBack = humanoidRootPart.CFrame * CFrame.new(size.X/2, 0, size.Z/2)
    }
    ...
    local isVisible = true
    if visualsSettings.visibleCheck then
        local distance = (humanoidRootPart.Position - Camera.CFrame.Position).Magnitude
    ...
    local boxColor = isVisible and espBoxSettings.VisibleColor or espBoxSettings.NotVisibleColor
    
    -- Update lines
    local box = espBoxes[player]
    
    -- Top face
    box.topLine.From = screenCorners.topLeft.Position
    box.topLine.To = screenCorners.topRight.Position
    box.topLine.Visible = screenCorners.topLeft.OnScreen and screenCorners.topRight.OnScreen
    box.topLine.Color = boxColor
    
    -- Bottom face
    box.bottomLine.From = screenCorners.bottomLeft.Position
    box.bottomLine.To = screenCorners.bottomRight.Position
    box.bottomLine.Visible = screenCorners.bottomLeft.OnScreen and screenCorners.bottomRight.OnScreen
    box.bottomLine.Color = boxColor
    
    -- Left face
    box.leftLine.From = screenCorners.topLeft.Position
    box.leftLine.To = screenCorners.bottomLeft.Position
    box.leftLine.Visible = screenCorners.topLeft.OnScreen and screenCorners.bottomLeft.OnScreen
    box.leftLine.Color = boxColor
    
    -- Right face
    box.rightLine.From = screenCorners.topRight.Position
    box.rightLine.To = screenCorners.bottomRight.Position
    box.rightLine.Visible = screenCorners.topRight.OnScreen and screenCorners.bottomRight.OnScreen
    box.rightLine.Color = boxColor
    
    -- Connecting lines (3D effect)
    box.topLeftLine.From = screenCorners.topLeft.Position
    box.topLeftLine.To = screenCorners.topLeftBack.Position
    box.topLeftLine.Visible = screenCorners.topLeft.OnScreen and screenCorners.topLeftBack.OnScreen
    box.topLeftLine.Color = boxColor
    
    box.topRightLine.From = screenCorners.topRight.Position
    box.topRightLine.To = screenCorners.topRightBack.Position
    box.topRightLine.Visible = screenCorners.topRight.OnScreen and screenCorners.topRightBack.OnScreen
    box.topRightLine.Color = boxColor
    
    box.bottomLeftLine.From = screenCorners.bottomLeft.Position
    box.bottomLeftLine.To = screenCorners.bottomLeftBack.Position
    box.bottomLeftLine.Visible = screenCorners.bottomLeft.OnScreen and screenCorners.bottomLeftBack.OnScreen
    box.bottomLeftLine.Color = boxColor
    
    box.bottomRightLine.From = screenCorners.bottomRight.Position
    box.bottomRightLine.To = screenCorners.bottomRightBack.Position
    box.bottomRightLine.Visible = screenCorners.bottomRight.OnScreen and screenCorners.bottomRightBack.OnScreen
    box.bottomRightLine.Color = boxColor
end
...
spawn(function()
    while true do
        if espBoxSettings.Enabled and visualsSettings.enableESP then
            local currentTime = tick()
            
            if currentTime - lastESPBoxUpdate >= espBoxSettings.UpdateInterval then
                lastESPBoxUpdate = currentTime
